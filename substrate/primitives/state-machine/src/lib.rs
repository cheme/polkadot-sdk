...
mod execution {
	...
	/// Generate child storage read proof and return state root too.
	/// Warning this do not include the child trie root check and another
	/// proof on the top trie is needed to check it.
	pub fn prove_read_v2<'a, B, H, I>(
		backend: B,
		child_info: Option<&ChildInfo>,
		start: Option<(&[u8], bool)>,
		keys: I,
	) -> Result<(Vec<Vec<u8>>, H::Out), Box<dyn Error>>
	where
		B: AsTrieBackend<H>,
		H: Hasher,
		H::Out: Ord + Codec,
	...
	{
		let trie_backend = backend.as_trie_backend();
		prove_read_on_trie_backend_v2(trie_backend, child_info, start, keys)
	}
	...
	/// Generate storage read proof on pre-created trie backend.
	pub fn prove_read_on_trie_backend_v2<'a, S, H, I>(
		trie_backend: &TrieBackend<S, H>,
		child_info: Option<&ChildInfo>,
		start: Option<(&[u8], bool)>,
		keys: I,
	...
	{
		trie_backend
			.prove_read_on_trie_backend_v2(child_info, start, keys)
			.map_err(|e| Box::new(e) as Box<dyn Error>)
	}
	...
	pub fn read_proof_check_v2<'a, H, I>(
		root: H::Out,
		proof: Vec<Vec<u8>>,
		keys: I,
	...
	{
		use sp_trie::{
			verify_query_plan_iter, ProofKind, QueryPlan, QueryPlanItemRef, ReadProofItem,
		};
		...
		let query_plan = QueryPlan { items, kind, _ph: Default::default() };
		let verify_iter = verify_query_plan_iter::<sp_trie::LayoutV0<H>, _, _, _>(
			query_plan.into(),
			proof.into_iter(),
			Some(root.clone()),
		)
		...
		let mut result = HashMap::new();
		// Note that a lot more can be check but this is currently only use for tests.
		for item in verify_iter {
			match item.map_err(|e| Box::new(e) as Box<dyn Error>)? {
				ReadProofItem::Hash(key, hash) => {
					result.insert(key.to_vec(), HashOrValue::Hash(hash));
				},
				ReadProofItem::Value(key, value) => {
					result.insert(key.to_vec(), HashOrValue::Value(value));
				},
				ReadProofItem::NoValue(key) => {
					result.insert(key.to_vec(), HashOrValue::None);
				},
				ReadProofItem::StartPrefix(_prefix) => (),
				ReadProofItem::EndPrefix => (),
				ReadProofItem::Halted(_resume) => (),
			}
		}
		Ok(result)
	}
...
mod tests {
	...
	fn compact_multiple_child_trie_inner(state_version: StateVersion) -> usize {
		...
		let accesses_2 =
			[(&b"k"[..], true, false), (&b"key1"[..], false, false), (&b"rando"[..], true, false)];
		let remote_backend = TrieBackendBuilder::new(&remote_storage, remote_root).build();
		let (remote_proof_2, root_2) = prove_read_v2(
			remote_backend,
			Some(&child_info1),
			None,
			accesses_2.iter().map(|(k, h, p)| (*k, *h, *p)),
		)
		.unwrap();
		...
		let local_result1 = read_proof_check_v2::<BlakeTwo256, _>(
			root_2,
			remote_proof_2,
			accesses_2.iter().map(|(k, h, p)| (*k, *h, *p)),
		)
