...
mod execution {
	...
	pub struct StateMachine<'a, B, H, H2, Exec>
	where
		H: Hasher,
		H2: Hashers,
		B: Backend<H>,
	{
		...
		context: CallContext,
		_ph: sp_std::marker::PhantomData<H2>,
	}

	impl<'a, B, H, H2, Exec> Drop for StateMachine<'a, B, H, H2, Exec>
	where
		H: Hasher,
		H2: Hashers,
		B: Backend<H>,
	...
	impl<'a, B, H, H2, Exec> StateMachine<'a, B, H, H2, Exec>
	where
		H: Hasher,
		H2: Hashers,
		...
		B: Backend<H>,
	{
		...
		pub fn execute(
			...
			let mut ext =
				Ext::<_, H2, _>::new(self.overlay, self.backend, Some(&mut self.extensions));
	...
	pub fn prove_execution<B, H, H2, Exec>(
		backend: &mut B,
		...
		Exec: CodeExecutor + Clone + 'static,
	{
		...
		prove_execution_on_trie_backend::<_, _, H2, _>(
			trie_backend,
			overlay,
			exec,
			method,
	...
	/// Note: changes to code will be in place if this call is made again. For running partial
	/// blocks (e.g. a transaction at a time), ensure a different method is used.
	pub fn prove_execution_on_trie_backend<S, H, H2: Hashers, Exec>(
		...
		Exec: CodeExecutor + 'static + Clone,
	{
		...
		let result = StateMachine::<_, H, H2, Exec>::new(
			&proving_backend,
			overlay,
			exec,
			method,
	...
	pub fn execution_proof_check<H, H2, Exec>(
		root: H::Out,
		proof: StorageProof,
		overlay: &mut Changes<H>,
		exec: &Exec,
		...
		H::Out: Ord + 'static + codec::Codec,
	{
		...
		execution_proof_check_on_trie_backend::<_, H2, _>(
			&trie_backend,
			overlay,
			exec,
			method,
	...
	pub fn execution_proof_check_on_trie_backend<H, H2, Exec>(
...
pub mod fuzz {
	...
	impl Overlay {
		...
		fn extract_state(&self) -> HashMap<u8, Vec<u8>> {
			...
			change
				.drain_transient_committed()
				.unwrap()
				.1
				.into_iter()
				.map(|(meta, chunks)| {
					let blob = crate::overlayed_changes::blob_from_chunks(chunks, meta.size);
...
mod tests {
	...
	use crate::{execution::CallResult, in_memory_backend::new_in_mem, testing::DummyHashers};
	...
	use sp_runtime::{traits::BlakeTwo256, IoHashers};
	use sp_trie::{
	...
	fn execute_works_inner(state_version: StateVersion) {
		...
		let mut state_machine = StateMachine::<_, _, DummyHashers, _>::new(
			&backend,
	...
	fn prove_execution_and_proof_check_works_inner(state_version: StateVersion) {
		...
		let (remote_result, remote_proof) = prove_execution::<_, _, DummyHashers, _>(
			&mut remote_backend,
			&mut Default::default(),
			&executor,
			"test",
		...
		// check proof locally
		let local_result = execution_proof_check::<BlakeTwo256, DummyHashers, _>(
			remote_root,
			remote_proof,
			&mut Default::default(),
			&executor,
	...
	fn clear_prefix_in_ext_works() {
		...
		{
			let mut ext = Ext::<_, IoHashers, _>::new(&mut overlay, backend, None);
