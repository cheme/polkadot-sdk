...
pub use crate::{
	...
	ext::Ext,
	overlayed_changes::{
		BlobsCollection, Changes, ChildStorageCollection, IndexOperation, OffchainChanges,
		OffchainChangesCollection, OrdMapsCollection, StorageChanges, StorageCollection,
			...
			let (child_info, depth) = if let Some(storage_key) = child_key.as_ref() {
				...
				(
					Some(match ChildType::from_prefixed_key(storage_key) {
						...
						Some((ChildType::OrderedMap, _))
						| Some((ChildType::Blob, _)) => {
							return Err(Box::new("Invalid range start child trie key, reserved for transient storage."))
						},
			...
			let (child_info, depth) = if let Some((storage_key, state_root)) = child_key.as_ref() {
				...
				(
					Some(match ChildType::from_prefixed_key(storage_key) {
						Some((ChildType::Default, storage_key)) =>
							ChildInfo::new_default(storage_key),
Some((ChildType::OrderedMap, _)) | Some((ChildType::Blob, _)) => return Err(Box::new("Invalid range start child trie key, reserved for transient storage.")),
						None => return Err(Box::new("Invalid range start child trie key.")),
					}),
					2,
				)
...
pub mod fuzz {
	...
	/// Movable storage operations.
	#[derive(Arbitrary, Clone, Copy, Debug)]
	pub enum Ops {
		StartTransaction,
		RollbackTransaction,
		CommitTransaction,
		SetValue(u8, u8), // key storage, value
		Delete(u8),       // key storage
		RemoveValue(u8),  // key storage
		Create(u8),       // key storage
		CopyTo(u8, u8),
		MoveTo(u8, u8),
		Hash(u8),
		SetValueAt(u8, u8, u16), // key storage, value, at.
	}

	#[derive(Debug, PartialEq, Eq)]
	struct Simulate {
		data: Vec<HashMap<u8, Vec<u8>>>,
		conf: FuzzConf,
	}

	/// Define some variant for fuzzing.
	#[derive(Debug, Clone, Copy, PartialEq, Eq)]
	pub struct FuzzConf {
		/// size of content in number of repeated bytes.
		pub content_size: usize,
		/// test caching (hash but actually here we just
		/// write content)
		pub do_hash: bool,
		/// allow set at random index.
		pub set_at: bool,
	}
	...
	struct ApplyReply {
		ignore: bool,
		with_hash: Option<Vec<u8>>,
	}

	impl Simulate {
		fn new(conf: FuzzConf) -> Self {
			Self { data: vec![Default::default()], conf }
		}
		// if return false, we should not test on the actual overlay.
		fn apply(&mut self, ops: Ops) -> ApplyReply {
			let mut with_hash = None;
			match ops {
				Ops::StartTransaction => {
					let new = self.data.last().cloned().unwrap_or_else(HashMap::new);
					self.data.push(new);
				},
				Ops::RollbackTransaction =>
					if self.data.len() == 1 {
						return ApplyReply { ignore: true, with_hash: None }
					} else {
						self.data.pop();
					},
				...
				Ops::SetValue(k, v) => {
					self.data.last_mut().map(|storages| {
						if storages.contains_key(&k) {
							storages.insert(k, vec![v; self.conf.content_size]);
						}
					});
				},
				Ops::RemoveValue(k) => {
				...
				Ops::CopyTo(f, t) => {
					self.data.last_mut().map(|storages| {
						if let Some(s) = storages.get(&f) {
							storages.insert(t, s.clone());
						}
					});
				},
				Ops::MoveTo(f, t) => {
					self.data.last_mut().map(|storages| {
						if let Some(s) = storages.remove(&f) {
							storages.insert(t, s);
						}
					});
				},
				Ops::SetValueAt(k, v, o) =>
					if self.conf.set_at {
						self.data.last_mut().map(|storages| {
							if let Some(dest) = storages.get_mut(&k) {
								let o = o as usize;
								let o_end = o + self.conf.content_size;
								if o <= dest.len() {
									if o_end > dest.len() {
										dest.resize(o_end, 0);
									}
				...
				Ops::Hash(n) =>
					if self.conf.do_hash {
						self.data.last_mut().map(|storages| {
							with_hash = storages.get(&n).cloned();
						});
					},
			}
			ApplyReply { ignore: false, with_hash }
		}
	}

	struct Overlay {
		data: Changes<sp_runtime::traits::BlakeTwo256>,
		conf: FuzzConf,
	}
	...
	impl Overlay {
		fn new(conf: FuzzConf) -> Self {
			...
			change.enter_runtime().unwrap();
			Self { data: change, conf }
		}

		fn extract_state(&self) -> HashMap<u8, Vec<u8>> {
			let mut change = self.data.clone();

			change.exit_runtime(true).unwrap();
			change
				.drain_committed()
				.3
				.into_iter()
				.filter_map(|(meta, chunks)| {
					if meta.removed {
						return None
					}
					let k: u8 = meta.infos.name[0];
					let value = overlayed_changes::blob_from_chunks(chunks, meta.size);
					Some((k, value))
				})
				.collect()
		}
		...
		fn apply(&mut self, ops: Ops) -> ApplyReply {
			...
			match ops {
				...
				Ops::RollbackTransaction => {
					self.data.rollback_transaction().unwrap();
				},
				Ops::CommitTransaction => {
					self.data.commit_transaction().unwrap();
				},
				Ops::SetValue(k, v) => {
					self.data.blob_truncate(&[k], 0);
					self.data.blob_set(&[k], &vec![v; self.conf.content_size], 0);
				},
				Ops::RemoveValue(k) => {
					self.data.blob_truncate(&[k], 0);
				},
				Ops::Delete(k) => {
					let child_info = ChildInfo::new_blob(
						&[k],
						Some(sp_core::storage::transient::Mode::Archive),
						None,
					);
					self.data.clear_child_storage(&child_info);
				},
				Ops::Create(k) => {
					let child_info = ChildInfo::new_blob(
						&[k],
					...
					let clear_existing = true;
					self.data.update_storage_info(child_info, clear_existing);
				},
				Ops::CopyTo(f, t) => {
					let child_info = ChildInfo::new_blob(
						&[f],
					...
					self.data.storage_copy(&child_info, &[t]);
				},
				Ops::MoveTo(f, t) => {
					let child_info = ChildInfo::new_blob(
						&[f],
					...
					self.data.storage_move(&child_info, &[t]);
				},
				Ops::SetValueAt(k, v, o) =>
					if self.conf.set_at {
						self.data.blob_set(&[k], &vec![v; self.conf.content_size], o as u32);
					},
				Ops::Hash(n) =>
					if self.conf.do_hash {
						let mut do_cache = None;
						// code from ext
						if let Some((chunks, info)) =
							self.data.blobs.get(&[n][..]).and_then(|b| b.chunks())
						{
							with_hash = info.cached_hash.clone();
							if with_hash.is_none() {
								let mut hash = Vec::new();
								for chunk in chunks {
									hash.extend_from_slice(chunk);
								}
								do_cache = Some(hash.clone());
								// simul always got a value so get it too
								with_hash = Some(hash);
							}
						}
						if let Some(hash) = do_cache {
							self.data.cache_blob_hash(&[n], hash);
						}
					},
			}
			ApplyReply { ignore: false, with_hash }
	...
	/// fuzz a list of ops.
	pub fn fuzz_movable(test: &[Ops], conf: FuzzConf) {
		let mut sim = Simulate::new(conf);
		let mut change = Overlay::new(conf);
		for op in test.iter() {
			let sim_result = sim.apply(*op);
			if !sim_result.ignore {
				let result = change.apply(*op);
				assert_eq!(sim_result, result);
			}
		}
		assert_eq!(&change.extract_state(), sim.data.last().unwrap());
	}
}

#[cfg(test)]
mod tests {
	use super::{backend::AsTrieBackend, ext::Ext, fuzz::*, *};
	...
	#[test]
	fn transient_map_storage_works() {
		let child_info = ChildInfo::new_ordered_map(
			b"sub1",
		...
		assert!(!ext.set_child_storage(child_info, b"abc", b"def"));
		assert_eq!(ext.child_storage(child_info, b"abc", 0, None), None);
		assert!(ext.update_storage_info(child_info.clone(), true));
		assert!(ext.set_child_storage(child_info, b"abc", b"def"));
		assert_eq!(ext.child_storage(child_info, b"abc", 0, None), Some(b"def"[..].into()));
		let _ = ext.kill_child_storage(child_info, None, None);
		assert_eq!(ext.child_storage(child_info, b"abc", 0, None), None);
	...
	fn blob_changes() {
		...
		let initial: BTreeMap<Vec<u8>, Vec<u8>> = Default::default();
		...
		overlay.start_transaction();
		// blob not created
		assert!(!overlay.blob_set(blob1.storage_key(), &[0], 0));
		// create blob
		assert!(overlay.update_storage_info(blob1.clone(), true));
		// out of range
		assert!(!overlay.blob_set(blob1.storage_key(), &[0], 1));
		// ok
		assert!(overlay.blob_set(blob1.storage_key(), &[0], 0));
		assert!(overlay.blob_set(blob1.storage_key(), &[1, 2], 1));
		{
			let mut ext = Ext::new(&mut overlay, backend, None);
			ext.blob_new(blob2.storage_key(), sp_core::storage::transient::Mode::Drop);
			assert_eq!(ext.blob_get(blob1.storage_key(), Some(1), 1), Some(vec![1].into()));
			assert!(ext.blob_exists(blob1.storage_key()));
			assert!(!ext.blob_exists(blob3.storage_key()));
			ext.storage_start_transaction();
			assert!(ext.blob_clone(blob1.storage_key(), blob3.storage_key()));
			assert_eq!(ext.blob_get(blob3.storage_key(), Some(8), 1), Some(vec![1, 2].into()));
			assert!(ext.blob_delete(blob1.storage_key()));
			assert!(!ext.blob_exists(blob1.storage_key()));
			assert_eq!(ext.blob_get(blob1.storage_key(), Some(8), 1), None);
			ext.storage_start_transaction();
			assert!(ext.blob_rename(blob3.storage_key(), blob1.storage_key()));
			assert_eq!(ext.blob_get(blob1.storage_key(), Some(1), 1), Some(vec![1].into()));
			assert!(!ext.blob_exists(blob3.storage_key()));
			assert_eq!(ext.blob_get(blob3.storage_key(), Some(8), 1), None);
			assert_eq!(ext.blob_len(blob1.storage_key()), Some(3));
			assert!(!ext.blob_truncate(blob1.storage_key(), 5));
			assert!(ext.blob_truncate(blob1.storage_key(), 1));
			assert_eq!(ext.blob_get(blob1.storage_key(), None, 0), Some(vec![0].into()));
			assert_eq!(ext.blob_len(blob1.storage_key()), Some(1));
			use hash_db::Hasher;
			let value_hash = BlakeTwo256::hash(&[0]);
			assert_eq!(
				ext.blob_hash32(
					blob1.storage_key(),
			...
			let value_hash = BlakeTwo256::hash(value.as_slice());
			assert!(ext.blob_set(blob1.storage_key(), value.as_slice(), 0));
			assert_eq!(
				ext.blob_hash32(
					blob1.storage_key(),
					sp_core::storage::transient::Hash32Algorithm::Blake2b256
	...
	fn movable_storage_simple() {
		let tests = &[
			...
			vec![
				...
				Ops::CommitTransaction,
				Ops::RollbackTransaction,
				// on last change of 192 is dirty size 2 instead of 1?? -> on commit the is
				// previous moved from looks skeezed.
			],
			vec![
				Ops::Create(255),
				Ops::StartTransaction,
			...
			vec![Ops::Create(94), Ops::Create(193), Ops::Create(192), Ops::CopyTo(193, 209)],
			vec![Ops::Create(19), Ops::Delete(19)],
			vec![Ops::Create(19), Ops::MoveTo(19, 2)],
			vec![Ops::StartTransaction, Ops::Create(19)],
			vec![Ops::Create(19)],
			vec![Ops::StartTransaction, Ops::RollbackTransaction],
		];
		let conf = FuzzConf { content_size: 6, do_hash: false, set_at: false };
		for test in tests {
			fuzz_movable(test, conf);
		}
	}

	#[test]
	fn movable_storage_append() {
		let tests = &[
			vec![Ops::Create(107), Ops::SetValue(107, 235)],
			vec![
				Ops::Create(70),
				Ops::SetValue(70, 70),
				Ops::SetValueAt(70, 70, 5),
				Ops::SetValue(70, 70),
			],
			vec![Ops::Create(255), Ops::SetValueAt(255, 244, 0)],
		];
		let conf = FuzzConf { content_size: 277, do_hash: true, set_at: true };
		for test in tests {
			fuzz_movable(test, conf);
		}
	}
