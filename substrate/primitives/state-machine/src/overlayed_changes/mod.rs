...
use sp_core::{
	...
	storage::{
		transient::{Hash32Algorithm, Mode, Root32Structure},
...
/// TODO if going global variable direction: split in two struct (or not)
pub struct Changes<H: Hasher> {
	...
	/// Oredered maps to archive, this is a flat storage (no transaction).
	/// Can only be written.
	archive_blobs: Map<Name, (BlobMeta, Vec<u8>)>,

	/// Blob to archive, this is a flat storage (no transaction).
	/// Can only be written.
	archive_ordered_maps: Map<Name, (OrdMapMeta, Vec<(Vec<u8>, Option<Vec<u8>>)>)>,
}

impl<H: Hasher> Changes<H> {
	/// Put a chunk to be archive.
	pub fn blob_archive_chunk(&mut self, name: &[u8], chunk: &[u8]) {
		let entry = self.archive_blobs.entry(name.to_vec()).or_insert_with(|| {
			(
				MovableMeta {
					infos: TransientInfo {
						name: name.to_vec(),
						mode: Some(Mode::Archive),
						algorithm: None,
					},
					size: 0,
					cached_hash: None,
		...
		entry.0.size += chunk.len();
		entry.1.extend_from_slice(chunk);
	}

	/// Put a blob hash to be archive.
	pub fn blob_archive_hash(&mut self, name: &[u8], hash: &[u8], algo: Hash32Algorithm) {
		let entry = self.archive_blobs.entry(name.to_vec()).or_insert_with(|| {
			(
				MovableMeta {
					infos: TransientInfo {
		...
		entry.0.infos.algorithm = Some(algo);
		entry.0.cached_hash = Some(hash.to_vec());
	}

	/// Put a map key value to be archive.
	pub fn map_archive_item(&mut self, name: &[u8], key: &[u8], value: &[u8]) {
		let entry = self.archive_ordered_maps.entry(name.to_vec()).or_insert_with(|| {
			(
				MovableMeta {
					infos: TransientInfo {
						name: name.to_vec(),
		...
		entry.0.size += 1;
		entry.1.push((key.to_vec(), Some(value.to_vec())));
	}

	/// Put a map root to be archive.
	pub fn map_archive_root(&mut self, name: &[u8], root: &[u8], algo: Root32Structure) {
		let entry = self.archive_ordered_maps.entry(name.to_vec()).or_insert_with(|| {
			(
				MovableMeta {
					infos: TransientInfo {
		...
		entry.0.infos.algorithm = Some(algo);
		entry.0.cached_hash = Some(root.to_vec());
	}
}
...
impl<H: Hasher> Default for Changes<H> {
	fn default() -> Self {
		Self {
			...
			archive_ordered_maps: Default::default(),
			blobs: Default::default(),
			archive_blobs: Default::default(),
...
impl<H: Hasher> Clone for Changes<H> {
	fn clone(&self) -> Self {
		Self {
			...
			archive_ordered_maps: self.archive_ordered_maps.clone(),
			blobs: self.blobs.clone(),
			archive_blobs: self.archive_blobs.clone(),
...
impl<H: Hasher> sp_std::fmt::Debug for Changes<H> {
	fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
		f.debug_struct("Changes")
			...
			.field("archive_ordered_maps", &self.archive_ordered_maps)
			.field("blobs", &self.blobs)
			.field("archive_blobs", &self.archive_blobs)
...
impl<H: Hasher> Changes<H> {
	...
	pub fn child_storage(
		...
		let value = match child_info {
			...
			ChildInfo::Blob(info) => {
				...
				Some(map.changes.storage(start, limit, infos.size, &self.stats).into())
			},
		};
		Some(value)
	}
	...
	/// Set byte data at a given offset (modification local to current transactional layer).
	/// Return true if value was set.
	/// Return false when not set (e.g. offset is out of bounds).
	pub fn blob_set(&mut self, name: &[u8], value: &[u8], offset: u32) -> bool {
	...
	/// Truncate blob to a new length (modification local to current transactional layer).
	/// Return true if something was changed.
	pub fn blob_truncate(&mut self, name: &[u8], new_len: u32) -> bool {
		let Some(previous_len) = self.blob_len(name) else { return false };
	...
	pub(crate) fn clear_child_prefix(&mut self, child_info: &ChildInfo, prefix: &[u8]) -> u32 {
		if let ChildInfo::Default(_) = child_info {
			self.mark_dirty();
		}
	...
	pub(crate) fn drain_committed(
		...
		impl Iterator<Item = (BlobMeta, Blob)>,
	) {
		...
		(
			...
			take(&mut self.archive_ordered_maps)
				.into_values()
				.map(|(infos, changes)| (infos, changes.into_iter())),
			take(&mut self.archive_blobs).into_values(),
		)
	}

	/// Consume all transient changes (blobs + ordered_map) and return them.
	///
	/// This should after be injected (only archived transient are return here)
	/// through archive storage host functions.
	///
	/// Return `None` when `transaction_depth() > 0`
	/// TODO in frame support call it and inject in util function.
	pub fn drain_transient_committed(
		&mut self,
	) -> Option<(
		impl Iterator<Item = (OrdMapMeta, impl Iterator<Item = (StorageKey, Option<StorageValue>)>)>,
		impl Iterator<Item = (BlobMeta, impl Iterator<Item = [u8; BLOB_CHUNK_SIZE]>)>,
	)> {
		if !self.context.can_drain_committed() {
			return None
		}
		use sp_std::mem::take;
		self.context = Default::default(); // Return to client (legacy behavior).
		...
		H::Out: Ord + Encode + 'static,
	{
		...
		Ok(StorageChanges {
			...
			blobs_changes: blobs_changes.map(|(info, blob)| (info, blob)).collect(),
...
impl<H: Hasher> Changes<H> {
	/// Calculate root of an ordered map.
	pub fn ordered_map_storage_root(&mut self, child_info: &ChildInfo) -> Option<Vec<u8>>
	where
		H: Hasher,
		H::Out: Encode,
	{
		if let ChildInfo::OrderedMap(..) = child_info {
			let result = if let Some((data, info)) =
				self.ordered_map_changes(child_info.storage_key())
			{
				...
				match info.infos.algorithm {
					Some(Root32Structure::SubstrateDefault) => {
						...
						sp_trie::child_trie_root::<sp_trie::LayoutV1<H>, _, _, _>(delta).encode()
					},
					None => return None,
				}
			} else {
			...
			self.cache_ordered_map_root(child_info.storage_key(), result.clone());
			return Some(result)
		}
		None
	}
...
impl<H2: Hasher> Changes<H2> {
	/// Calculate hash of a blob.
	pub fn blob_hash<H: Hashers>(&mut self, child_info: &ChildInfo) -> Option<Vec<u8>> {
		if let ChildInfo::Blob(..) = child_info {
			let result = {
				let Some((mut chunks, info)) =
					self.blobs.get(child_info.storage_key()).and_then(|b| b.chunks())
				else {
					return None
				};
				...
				let result = if let Some(chunk) = chunks.next() {
					...
					if let Some(hash) = hasher_finalize!(self, state) {
						hash
					} else {
						drop_hasher!(self, Some(state));
						return None
			...
			self.cache_blob_hash(child_info.storage_key(), result.clone());
			return Some(result)
		}
		None
	}
...
mod tests {
	use super::*;
	use crate::{ext::Ext, new_in_mem, testing::DummyHashers, InMemoryBackend};
	...
	fn overlayed_storage_root_works() {
		...
		{
			let mut ext = Ext::<_, DummyHashers, _>::new(&mut overlay, &backend, None);
	...
	fn transient_map_storage_works() {
		...
		let child_info = &child_info;
		let mut overlay = Changes::<Blake2Hasher>::default();
		assert!(!overlay.set_child_storage(child_info, b"abc", Some(b"def")));
		assert_eq!(overlay.child_storage(child_info, b"abc", 0, None), None);
		assert!(overlay.update_storage_info(child_info.clone(), true));
		assert!(overlay.set_child_storage(child_info, b"abc", Some(b"def")));
		assert_eq!(
			overlay.child_storage(child_info, b"abc", 0, None).flatten(),
			Some(Cow::Borrowed(&b"def"[..]))
		);
		let _ = overlay.clear_child_storage(child_info);
		assert_eq!(overlay.child_storage(child_info, b"abc", 0, None).flatten(), None);
	}
}
